...
import fs
from pathname import Pathname, PathnameMatch, PathnameComponent
...
_XDG_CONFIG_HOME_PREFIX_PATHNAME: Pathname = Pathname("$XDG_CONFIG_HOME")
        ...
        for line in instructions_file:
            ...
            if current_instruction_group is not None:
                ...
                if match is not None:
                    ...
                    pmatch: PathnameMatch | None = None

                    target_pathname.match_prefix(_HOME_VAR_PREFIX_PATHNAME)
                    if target_pathname.startswith(_HOME_VAR_PREFIX_PATHNAME):
                        target_pathname = target_pathname.replace_prefix(
                            _HOME_VAR_PREFIX_PATHNAME,
                            home_pathname,
                        )
                    elif target_pathname.startswith(_XDG_CONFIG_HOME_PREFIX_PATHNAME):
                        target_pathname = target_pathname.replace_prefix(
                            _XDG_CONFIG_HOME_PREFIX_PATHNAME,
            ...
            if match is not None:
                include_pathname: Pathname = Pathname.create_normalized(match.group("pathname"))
