...
if [ -f %%systemBashRcFilePath:String%% ]; then
...
if command -v git > '/dev/null'; then
	if [ -f %%systemBashGitPromptFilePath:String%% ]; then
		. %%systemBashGitPromptFilePath:String%%
	fi

	# pre-sourcing git's bash completion so that `__git_complete` is available
	if [ -f %%gitBashCompletionFilePath:String%% ]; then
		. %%gitBashCompletionFilePath:String%%
	fi
fi
